...
impl Token {
    ...
    pub fn update(&mut self, dt: f32, rl: &RaylibHandle, mouse_pos: Vector2) -> bool {
        if !self.is_dragging && rl.is_mouse_button_pressed(MouseButton::MOUSE_BUTTON_LEFT)
            && Rectangle::new(
        ...
        if self.is_dragging && (rl.is_mouse_button_pressed(MouseButton::MOUSE_BUTTON_RIGHT)
